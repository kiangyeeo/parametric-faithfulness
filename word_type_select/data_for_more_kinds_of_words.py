...
class SegmentOTFDataset(Dataset):
    def __init__(self, forget, retain, tokenizer, stepwise=False, pos_filter=False, step_idx=0, word_type_group=None):
        self.forget = forget # Either a single sentence or a set of atomic statements
        self.retain = retain
    ...
    def __getitem__(self, idx):
        ...
        (E_f, L_f, A_f, T_f) = qcot_encoder(self.tokenizer, prompt, forget_sample['completion'], 
                                              pos_filter=self.pos_filter, nlp=self.NLP, 
                                              word_type_group=self.word_type_group)
...
def cot_to_otfd(target, all, tokenizer, n=4, strategy='full', stepwise=True, step_idx=0, pos=False, word_type_group=None):
    ...
    if strategy == 'full':
        ...
        return SegmentOTFDataset(target, retain, tokenizer, stepwise, pos_filter=pos, word_type_group=word_type_group)
        ...
        return SegmentOTFDataset(targets, retain, tokenizer, stepwise, step_idx=step_idx, pos_filter=pos, word_type_group=word_type_group)
