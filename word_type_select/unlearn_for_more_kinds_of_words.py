...
def unlearn_single(model_id, tokenizer, args, target, step_idx, cots_train, cots_verify, dh, instance_idx):
    ...
    dataset = cot_to_otfd(target, cots_train, tokenizer, strategy=args.strategy, stepwise=args.stepwise, step_idx=step_idx, pos=args.pos, word_type_group=args.word_type_group)
    ...
    if NT <= 1:
         print("-"*20)
         wt_group = getattr(args, 'word_type_group', None)
         if NT == 0:
             skip_reason = f"No target words for word_type_group='{wt_group}' in this step"
         else:
...
def make_parser():
    ...
    parser.add_argument('--word_type_group', type=str, default=None,
                        help="Vocabulary type group to unlearn: entity, attribute, action, function, modifier, all_content, random")
    parser.add_argument('--atomic', action='store_true', help="Use atomic statement segmentation for CoT steps.")
...
def main():
    ...
    wt_suffix = "" if args.word_type_group is None else f"_wt={args.word_type_group}"
    logfile_name = f"{args.method}_{args.strategy}_s={args.stepwise}_lr={str(args.lr)}{lambda_suffix}{wt_suffix}_rs={args.seed}_pos={args.pos}_ff2={args.ff2}.out"
    ...
    for idx, target in enumerate(cots_train[:N_unlearn]):
        ...
        for step_idx in range(n_steps):
          ...
          instance_info = {
              ...
              'word_type_group': args.word_type_group,
          }
          ...
          if results is None:
              ...
              skip_log = {
                  'id': target['id'],
                  'step_idx': step_idx,
                  'cot_step': target['segmented_cot'][step_idx] if args.stepwise else target['cot'],
                  'word_type_group': args.word_type_group,
                  'skip_reason': skip_reason,
              }
              skip_log_path = resdir + logfile_name.replace('.out', '_skipped.jsonl')
              store(skip_log, skip_log_path)
              continue
    ...
    print("=" * 50)
    print(f"  Total instance-steps: {skip_stats['total']}")
    print(f"  Completed:            {skip_stats['completed']}")
    print(f"  Skipped (no targets): {skip_stats['skipped_no_targets']}")
    print(f"  Skipped (few targets):{skip_stats['skipped_few_targets']}")
    if skip_stats['total'] > 0:
        skip_rate = (skip_stats['skipped_no_targets'] + skip_stats['skipped_few_targets']) / skip_stats['total'] * 100
